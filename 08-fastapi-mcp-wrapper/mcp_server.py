#!/usr/bin/env python3
"""
Example 8: MCP Server Wrapper for FastAPI
Wraps the Task Manager FastAPI application with MCP protocol.
Makes REST API accessible through MCP tools.
"""
...
async def call_tool(name: str, arguments: dict) -> list[TextContent]:
    ...
    try:
        if name == "create_task":
            ...
            return [TextContent(
                type="text",
                text=f"Found {len(result)} task(s):\n\n" + "\n".join(task_list)
            )]
            ...
            return [TextContent(
                type="text",
                text=f"🗑️ Task deleted: {result['task']['title']}"
            )]
